...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const user = req.user;
    const { name, description, definition, category, triggerType, isActive } =
      req.body;
    ...
    try {
      ...
      if (workflow.isActive) {
        ...
        logger.info(
          `Skipping webhook setup for inactive workflow ${workflow.id}`,
        );
      }
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (workflow.isActive && !existingWorkflow.isActive) {
        ...
        try {
          logger.info(
            `Setting up webhooks for newly activated workflow ${workflow.id}`,
          );
          await setupWorkflowWebhooks(workflow, user.id);
          logger.info(`Webhook setup completed for workflow ${workflow.id}`);
        } catch (webhookError) {
...
async function setupWorkflowWebhooks(workflow: any, userId: string) {
  logger.info(
    `Starting webhook setup for workflow ${workflow.id}, user ${userId}`,
  );

  const definition = workflow.definition;

  // Check if workflow has GitHub trigger
  const hasGitHubTrigger = definition.blocks?.some(
    (block: any) => block.type === "github-trigger",
  );

  logger.info(
    `Workflow ${workflow.id} has GitHub trigger: ${hasGitHubTrigger}`,
  );
  ...
  if (hasGitHubTrigger) {
    ...
    logger.info(
      `GitHub integration found for user ${userId}: ${!!githubIntegration}`,
    );
    logger.info(
      `GitHub integration has access token: ${!!githubIntegration?.accessToken}`,
    );

    if (githubIntegration?.accessToken) {
      // Find the GitHub trigger block to get repository config
      const githubBlock = definition.blocks.find(
        (block: any) => block.type === "github-trigger",
      );
      ...
      if (githubBlock?.config?.repository) {
        try {
          logger.info(
            `Creating GitHub webhook for repo: ${githubBlock.config.repository}, workflow: ${workflow.id}`,
          );
          ...
          logger.info(
            `GitHub webhook created successfully for workflow ${workflow.id}:`,
            webhookResult,
          );
        } catch (error) {
          logger.error(
            `Failed to create GitHub webhook for workflow ${workflow.id}:`,
            error,
          );
          throw error;
        }
        ...
        logger.warn(
          `No repository configured in GitHub block for workflow ${workflow.id}`,
        );
      }
    } else {
